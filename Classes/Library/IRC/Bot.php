...
class Bot {
    ...
    private function main() {
		...
		do {
            ...
            $args = explode( ' ', $data );
            if(isset($args[1])){
                if(strtoupper($args[1]) == "PRIVMSG"){
                    $mx = $args;
                    unset($mx[0]);
                    unset($mx[1]);
                    unset($mx[2]);
                    $mx = implode(" ", $mx);
                    $this->log($args[2] . ": <" . $args[0] . "> $mx");
                } elseif(strtoupper($args[1]) == "JOIN"){
                    $this->log($args[0] . ": Joins " . $args[2]);
                } elseif(strtoupper($args[1]) == "QUIT"){
                    $mx = $args;
                    unset($mx[0]);
                    unset($mx[1]);
                    $mx = implode(" ", $mx);
                    $this->log($args[0] . ": Quit " . $mx);
                } elseif(strtoupper($args[1]) == "PART"){
                    $this->log($args[0] . ": Leaves " . $args[2]);
                } elseif(strtoupper($args[1]) == "KICK"){
                    $this->log($args[0] . ": Kicks " . $args[3]);
                    if($args[3] == $config["name"] && in_array($args[2], $config['channels'])){
                        $this->join_channel($args[2]);
                        $this->log("Rejoined channel $args[2]");
                    }
                } else {
                    $this->log($data);
                }
